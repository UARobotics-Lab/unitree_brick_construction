...
ensure_data_dirs()
archivo_json = path_results("rutina_ladrillo_prueba.json")
...
class ArmSequence:
    ...
    def move_to(self, updates: dict, duration=1.5, q_init_override=None):
        # Actualiza la posición objetivo sin borrar otras articulaciones ya fijadas
        if updates:
...
def main():
    ...
    # Modo automático: evita bloquear con input() entre pasos
    AUTO = True  # Cambia a False para avanzar manualmente
    ...
    for i, paso in enumerate(pasos):
        ...
        if not AUTO:
            ...
            if res.lower() == 'x':
                ...
                return
        # Ejecutar movimiento para este paso
        seq.move_to(posiciones_brazo, duration=dur, q_init_override=q_anterior)
        ...
        # En modo automático, breve pausa no bloqueante entre pasos
        if AUTO:
            time.sleep(0.05)
